 ...
 * Modern Dark Mode scientific workflow
 */
...
export function renderDashboardView({ currentAnalysis, onNavigate, onRunDetection, isProcessing = false }) {
      ...
      <div style="display: flex; justify-content: space-between; align-items: flex-start; margin-bottom: 24px; flex-wrap: wrap; gap: 16px;">
        <div>
          ...
          <p style="font-size: 14px; color: var(--text-muted); margin-top: 6px;">
            Upload side-scan sonar imagery to run detection with the trained <strong style="color: var(--text-secondary);">best.pt</strong> model.
          </p>
        </div>
        ...
        <div id="dashDropZone" style="border: 2px dashed var(--border-strong); border-radius: var(--radius-md); padding: 24px; text-align: left; cursor: pointer; background: rgba(0,0,0,0.2); display: flex; align-items: center; gap: 20px; transition: all 0.2s;">
          ...
          <div>
            ...
            <div style="font-size: 13px; color: var(--text-muted); margin-top: 4px;">
              Supports PNG, JPG, BMP · Evaluated securely via Render backend
            </div>
          </div>
        </div>
        ...
        <div class="panel">
          ...
          <div style="display: flex; flex-direction: column; gap: 12px; max-height: 440px; overflow-y: auto; padding-right: 8px;">
            ${detections.map(d => {
              ...
              return `
                <div style="background: var(--color-bg-subtle); border: 1px solid var(--border-medium); border-left: 3px solid var(--color-class-${classType}); border-radius: var(--radius-sm); padding: 16px;">
                  ...
                  <div style="font-size: 13px; color: var(--text-muted); display: flex; flex-direction: column; gap: 6px; font-family: var(--font-mono);">
                    <div style="display: flex; justify-content: space-between;">
                      <span>Box:</span>
                      <span style="color: var(--text-secondary);">[${d.bounding_box?.x1}, ${d.bounding_box?.y1}] → [${d.bounding_box?.x2}, ${d.bounding_box?.y2}]</span>
                    </div>
                    <div style="display: flex; justify-content: space-between;">
                      <span>Size:</span>
                      <span style="color: var(--text-secondary);">${d.width_pixels} × ${d.height_pixels} px</span>
                    </div>
                    ...
                    ` : ''}
                    ${d.anomaly_score !== null && d.anomaly_score !== undefined ? `
                      <div style="margin-top: 4px; padding-top: 4px; border-top: 1px dashed var(--border-strong); display: flex; justify-content: space-between;">
                        <span>Anomaly:</span>
                        <strong style="color: var(--color-warning);">${d.anomaly_score.toFixed(1)}% (${d.anomaly_assessment})</strong>
                      </div>
                    ` : ''}
                  </div>
                </div>
            ...
            ${totalCount === 0 ? `
              <div style="text-align: center; padding: 60px 20px; color: var(--text-muted);">
                ...
                <div style="font-size: 13px;">No objects detected above the confidence threshold.</div>
              </div>
            ` : ''}
          </div>
        </div>
    ...
    ` : `
      ...
      <div class="panel" style="text-align: center; padding: 80px 24px; color: var(--text-muted); background: linear-gradient(180deg, var(--color-bg-card) 0%, rgba(24,24,27,0.5) 100%); border: 1px dashed var(--border-medium);">
        ...
        <p style="font-size: 14px; max-width: 500px; margin: 0 auto; line-height: 1.6;">
          Upload a side-scan sonar image using the form above or select a sample to initialize the YOLO detection pipeline.
        </p>
      </div>
    `}
  `;
  ...
  runBtn.addEventListener('click', async () => {
    ...
    statusMsg.textContent = 'Initializing YOLO best.pt backend via Render...';
